...
const birthYearIsValid = (b) => {
    return 1920 <= b && b <= 2002;
};
const issueYearIsValid = (i) => {
    return 2010 <= i && i <= 2020;
};
const expirationYearIsValid = (e) => {
    return 2020 <= e && e <= 2030;
};
...
const hairColorIsValid = (h) => {
    return /^#([0-9a-f]){6}$/.test(h);
};
const eyesColorIsValid = (e) => {
    return /^(amb|blu|brn|gry|grn|hzl|oth)$/.test(e);
};
const passportIdIsValid = (p) => {
    return /\d{9}/.test(p);
};
